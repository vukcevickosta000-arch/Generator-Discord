...
def sfx():
    ...
    # concept heroes: attacks
    W("nyxara_rapier", mix(hp(whoosh(0.14, 2500, 8000, 0.7), 1500), at(mix(metal_hit(2400, 0.2, 0.35), flesh_hit(0.1, 0.5)), 0.1, int(0.4 * SR))))
    W("malgrave_bolt", reverb(mix(osc(sweep(600, 220, 0.3), 0.3, "tri") * exp_env(int(0.3 * SR), 0.1) * 0.5, debris(0.25, 0.4, 6)), 0.4, 0.25))
    W("ardyn_mace", mix(whoosh(0.28, 300, 2200, 0.6), at(mix(thud(75, 0.45, 1.1), metal_hit(750, 0.45, 0.5), bell(note("A5"), 0.4, 0.15) * 0.15), 0.18, int(0.8 * SR))))
    W("fenrax_claw", mix(hp(whoosh(0.16, 2500, 7000, 0.7), 1500), flesh_hit(0.14, 0.8), growl(0.2, 130, 0.25, "eh")))
    W("morwen_bolt", reverb(mix(osc(sweep(700, 1200, 0.25), 0.25, "sine") * exp_env(int(0.25 * SR), 0.08) * 0.4, bp(noise(0.3), 500, 2500) * exp_env(int(0.3 * SR), 0.1) * 0.4), 0.5, 0.3))
    W("thael_club", mix(whoosh(0.35, 200, 1500, 0.7), at(mix(thud(65, 0.5, 1.2), bp(noise(0.3), 200, 900) * exp_env(int(0.3 * SR), 0.06) * 0.7), 0.25, int(0.9 * SR))))
    # concept heroes: casts
    bats = np.zeros(int(0.7 * SR))
    for _ in range(26):
        f = R.uniform(4000, 9000)
        chirp = osc(sweep(f, f * 0.6, 0.04), 0.04, "sine") * exp_env(int(0.04 * SR), 0.01)
        bats += at(chirp * R.uniform(0.1, 0.4), R.uniform(0, 0.55), len(bats))
    W("nyxara_bats", reverb(mix(whoosh(0.5, 600, 3000, 0.6), bats, bp(noise(0.6), 150, 600) * env(int(0.6 * SR), 0.05, 0.1, 0.6, 0.2, 0.3) * 0.5), 0.5, 0.3))
    W("nyxara_kiss", reverb(mix(hp(whoosh(0.2, 2000, 6000, 0.6), 1200), at(mix(flesh_hit(0.2, 0.9), bell(note("E6"), 0.5, 0.2) * 0.25), 0.1, int(0.7 * SR))), 0.4, 0.25))
    W("nyxara_veil", reverb(mix(sweep_filter(noise(0.9), 3000, 300, kind="lp") * env(int(0.9 * SR), 0.02, 0.2, 0.5, 0.4, 0.3) * 0.7, choir([note("E4"), note("B4")], 0.9, "oo", 0.3)), 0.7, 0.4))
    W("nyxara_sentence", reverb(mix(whoosh(0.2, 400, 4000, 0.6), at(mix(metal_hit(1800, 0.5, 0.5), flesh_hit(0.35, 1.3), chord_pad([note("D3"), note("Ab3")], 1.0, "saw", 1200, 0.01, 0.5) * exp_env(int(1.0 * SR), 0.35)), 0.2, int(1.3 * SR))), 0.6, 0.3))
    W("malgrave_raise", reverb(mix(debris(1.0, 0.9, 40), rumble(1.0, 0.5, 220), choir([note("C3"), note("G3")], 1.0, "oo", 0.35)), 0.6, 0.3))
    W("malgrave_cage", reverb(mix(debris(0.7, 1.0, 50), thud(90, 0.5, 0.9), bp(noise(0.5), 1500, 5000) * exp_env(int(0.5 * SR), 0.1) * 0.3), 0.5, 0.25))
    W("malgrave_chill", reverb(mix(sweep_filter(noise(0.8), 5000, 800, kind="lp") * env(int(0.8 * SR), 0.02, 0.2, 0.6, 0.3, 0.3) * 0.8, bell(note("B5"), 0.8, 0.3) * 0.2), 0.6, 0.35))
    W("malgrave_legion", reverb(mix(rumble(2.0, 1.0, 140), debris(1.8, 0.8, 60), thud(40, 1.2, 1.2), choir([note("C2"), note("G2"), note("Eb3")], 2.0, "ah", 0.5)), 0.8, 0.35))
    W("ardyn_aegis", reverb(mix(bell(note("E5"), 1.0, 0.5) * 0.5, bell(note("B5"), 1.0, 0.5) * 0.3, hp(noise(0.4), 4000) * exp_env(int(0.4 * SR), 0.1) * 0.2), 0.7, 0.4))
    W("ardyn_consecrate", reverb(mix(choir([note("D4"), note("A4"), note("F#5")], 1.4, "ah", 0.6), thud(80, 0.4, 0.5)), 0.8, 0.4))
    W("ardyn_judgment", reverb(mix(bell(note("A4"), 0.6, 0.3) * 0.5, metal_hit(1200, 0.3, 0.3)), 0.4, 0.25))
    W("ardyn_crusade", reverb(mix(brass(note("D3"), 1.4, 0.8), brass(note("A3"), 1.4, 0.5), at(mix(thud(60, 0.5, 0.9), whoosh(0.6, 300, 2500, 0.7)), 0.5, int(1.6 * SR))), 0.7, 0.35))
    W("fenrax_pounce", mix(whoosh(0.4, 300, 3000, 0.8), growl(0.35, 110, 0.6, "ah"), at(flesh_hit(0.25, 1.0), 0.35, int(0.8 * SR))))
    howl_t = t_axis(1.6)
    howl_f = 280 + 160 * np.sin(np.clip(howl_t / 1.6, 0, 1) * np.pi) ** 0.6
    howl_x = formant(osc(howl_f * (1 + 0.01 * np.sin(2 * np.pi * 6 * howl_t)), 1.6, "saw"), "oo") * env(int(1.6 * SR), 0.15, 0.3, 0.8, 0.6, 0.5)
    W("fenrax_howl", reverb(howl_x * 0.8, 0.8, 0.4))
    W("fenrax_transform", reverb(mix(growl(1.4, 70, 1.0, "oh"), debris(0.8, 0.4, 20), at(howl_x * 0.5, 0.6, int(2.4 * SR))), 0.7, 0.35))
    W("morwen_hex", reverb(mix(osc(sweep(1200, 300, 0.4), 0.4, "tri") * exp_env(int(0.4 * SR), 0.15) * 0.4, at(mix(osc(sweep(180, 90, 0.2), 0.2, "square") * exp_env(int(0.2 * SR), 0.08) * 0.3, bp(noise(0.2), 300, 900) * 0.3), 0.3, int(0.7 * SR))), 0.5, 0.3))
    bubbles = np.zeros(int(1.0 * SR))
    for _ in range(18):
        f = R.uniform(300, 900)
        bubbles += at(osc(sweep(f, f * 2.2, 0.06), 0.06, "sine") * exp_env(int(0.06 * SR), 0.02) * R.uniform(0.2, 0.5), R.uniform(0, 0.9), len(bubbles))
    W("morwen_cauldron", reverb(mix(bubbles, thud(110, 0.3, 0.6), lp(noise(1.0), 500) * env(int(1.0 * SR), 0.05, 0.2, 0.4, 0.3, 0.4) * 0.3), 0.5, 0.3))
    W("morwen_curse", reverb(mix(choir([note("C4"), note("F#4")], 0.8, "eh", 0.4), bp(noise(0.6), 600, 2000) * exp_env(int(0.6 * SR), 0.2) * 0.4), 0.6, 0.35))
    W("morwen_witching", reverb(mix(bell(note("C4"), 2.0, 1.2) * 0.5, bell(note("F#4"), 2.0, 1.2) * 0.35, choir([note("C3"), note("F#3"), note("C4")], 2.0, "oo", 0.45)), 0.9, 0.45))
    W("thael_roots", mix(debris(0.8, 0.8, 30), rumble(0.8, 0.6, 180), bp(noise(0.7), 150, 700) * env(int(0.7 * SR), 0.05, 0.2, 0.6, 0.2, 0.3) * 0.5))
    W("thael_treant", reverb(mix(thud(60, 0.8, 1.0), debris(1.0, 0.6, 30), growl(1.0, 55, 0.5, "oh")), 0.6, 0.3))
    W("thael_grove", reverb(mix(bp(noise(1.2), 800, 4000) * env(int(1.2 * SR), 0.3, 0.2, 0.6, 0.3, 0.4) * 0.5, chord_pad([note("G3"), note("D4")], 1.2, "tri", 1500, 0.004, 0.4)), 0.8, 0.4))
    W("thael_wrath", reverb(mix(rumble(2.0, 1.1, 160), debris(1.8, 0.9, 70), growl(1.5, 50, 0.6, "ah")), 0.8, 0.35))
    # hero summons
    W("bone_hit", mix(debris(0.2, 0.7, 8), thud(140, 0.2, 0.5)))
    W("bone_break", mix(debris(0.7, 1.0, 35), thud(110, 0.3, 0.5)))
    W("wolf_bite", mix(flesh_hit(0.12, 0.8), growl(0.22, 170, 0.35, "eh")))
    W("wolf_death", reverb(mix(osc(sweep(900, 300, 0.5), 0.5, "saw") * exp_env(int(0.5 * SR), 0.2) * 0.25, hp(noise(0.5), 2000) * exp_env(int(0.5 * SR), 0.2) * 0.3), 0.6, 0.4))
