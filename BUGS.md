...
| B-010 | S2 | Bots / balance | Dawn wins most 5v5 bot games: 27 of 40 across seeds, and 26 of 31 with the same lineup on both teams. The map is point-symmetric (checked), and the alternating update order (F-036) did not remove the lean. Candidates still to test: lane assignment by lane name (Dawn's top is Dusk's bottom), Vharoth pit access, and bot code with team-specific coordinates. | `SimRunner -- 60 <seed> --mirror`. T-036 |
...
| F-036 | Blood War units updated in creation order every tick, and each wave spawns Dawn's creeps first, so Dawn's side struck first in every simultaneous exchange. | The update direction alternates every tick in both modes (it already did in RTS). This alone did not remove the Dawn lean in bot games (B-010). |
